...
class Plume:
    ...
    def compute_stoke_n_drag(
        ...
        self._v_stokes = (
            ...
            * pow(min(self._D_o_init, self.D_lim), 2.0)
    ...
    def compute_init_plume_params(self, env_param: EnvironmentParameters):
        ...
        self._L_moment = pow(self._F_moment, 3.0 / 4.0) / pow(
        ...
        self._D_o_init = self._D_g_init * pow(
...
class Simulation_Plume2D:
    ...
    def __init__(
        self,
        env_param: EnvironmentParameters = None,
        plume_data: Plume = None,
        verbose: bool = False,
        z_0=1.4,
        U_b=0.05,
    ):
        ...
        self._env_param = (
            EnvironmentParameters() if env_param is None else env_param
        )  ####WHY????+++++++
        if self._verbose:
    ...
    def run(self, runtime=300.0, steps=-1, output_interval=-1, timedelta=None):
        ...
        E_s = (
            ...
            * b_i
            * s_i
            * (v_i - (U_b * sin(rad_phi_i)))
            * rho_w
            * self._dt
        )  # shear entrainment [kg]; column Y
        E_f = (
            2.0 * b_i * s_i * (U_b * cos(rad_phi_i)) * rho_w * self._dt
        )  # force entrainment [kg]; column Z
        K = (
            self._env_param.kappa * sqrt(self.Cdb) * U_b * z_i
        )  # Karman value [m^2/s]; column AA
        E_t = (
            8.0 * PI * K * s_i * rho_w * self._dt if self._opt_d else 0.0
        ...
        k_i = (
            ...
            * (
                ...
                + 0.6
                * pow(Re, 0.5)
                * pow(self._env_param.schmidt_coeff, 1.0 / 3.0)
            )
            / D_g_i
            if D_g_i > 0.0
            else 0.0
        ...
        # Separation
        v_E_i = (
            E / (2.0 * PI * b_i * s_i * rho_w) / self._dt
        ...
        v_N_g_i = max(
            U_g_b_i * sin(rad_phi_i) - v_E_i, 0.0
        )  # org. vNgas, normal-oriented velocity [m/s] of gas bubbles, column AI
        S_g_i = (
            -2.0 * (G_g_i * v_N_g_i * self._dt) / (PI * b_i)
            if self._opt_sep
            else 0.0
        ...
        v_N_o_i = max(
            Uzz_o_i * sin(rad_phi_i) - v_E_i, 0.0
        )  # org. vNoil; normal-oriented velocity [m/s] of oil droplets, column AK
        S_o_i = (
            -2.0 * (G_o_i * v_N_o_i * self._dt) / (PI * b_i)
            if self._opt_sep
            else 0.0
        ...
        for i in range(1, self._N):
            ...
            E_t = (
                np.float64(8.0) * PI * K * s_i * rho_w * self._dt
                if self._opt_td
                else 0.0
            )
            ...
            v_N_g_i = max(U_g_b_i * sin(rad_phi_i) - v_E_i, 0.0)
            S_g_i = (
                -2.0 * (G_g_i * v_N_g_i * self._dt) / (PI * b_i)
                if self._opt_sep
                else 0.0
            )
            v_N_o_i = max(Uzz_o_i * sin(rad_phi_i) - v_E_i, 0.0)
            S_o_i = (
                -2.0 * (G_o_i * v_N_o_i * self._dt) / (PI * b_i)
                if self._opt_sep
                else 0.0
    ...
    def _calculate_init_sim_params_(self):
        ...
        self._lambda_calc = (
            sqrt(2.0 * self.a_j * self._tau_calc + 1.0) - 1.0
        ) / self.a_j
...
def get_concentration_sensor(
    ...
    if intersection_point == None:
        ...
        C_i = (
            C_i * 35
        )  # to transfer mg/L to ppmv. In our case we do need to be super accurate
    return C_i
...
def get_detection_plume(
    sensor: Sensor, leakage: Leakage, currents, runtime=300, tau=60
) -> Detection:
    ...
    for current in currents:
        ...
        # )
        concentration = get_concentration_sensor(
            current,
            sensor_coord,
            leak_coord,
            template_angle,
