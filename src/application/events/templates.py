...
from src.domain.events.sensors import EventType as SensorEventType
...
async def process(sensor_event: SensorEvent):
    ...
    last_sensor_event_is_critical: bool = TEMPLATES_CRITICAL_EVENT[
        sensor.template_id
    ][sensor.id]

    # `true` if any of sensor is `true`
    last_template_event_is_critical: bool = any(
        TEMPLATES_CRITICAL_EVENT[sensor.template_id].values()
    )

    if (
        last_sensor_event_is_critical
        and sensor_event.type == SensorEventType.OK
    ):
