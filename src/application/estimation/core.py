...
from src.domain.estimation.repository import EstimationsSummariesRepository
from src.domain.fields import Field, TagInfo
from src.domain.simulation import Detection
from src.domain.templates.repository import TemplatesRepository
...
class EstimationProcessor:
    ...
    def __init__(
        ...
        self.simulator_concentrations: NDArray = concentrations
    ...
    def process(self):
        ...
        logger.info(
            "All metrics exceed sigma-squared ? {}".format(result_tracker)
        )
        ...
        if result_tracker[0]:
            ...
            # logger.info("q1: {}; q2: {}; Q: {}".format(q1, q2, Q))
...
@transaction
async def process(detections: list[Detection]):
    first_detection_rate = detections[0]
    ...
    # TODO: Move to the functional-based approach
    estimation_processor = EstimationProcessor(
        ...
        concentrations=np.array(
            [detection.concentrations for detection in detections]
        ),
        sensor_number=tag_info.sensor_number - 1,
    )

    create_schema: EstimationSummaryUncommited = estimation_processor.process()
