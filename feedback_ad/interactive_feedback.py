...
class sensor_stream:
    ...
    def __init__(self, ts_array, m, start_stop_list):
        ...
        self.max_dis_fb = self.max_dis # new max dist that works only for feedbacks
    ...
    def track_data(self, datapoint, threshold=288):
        ...
        #Part relevant for procedure:
            if self.global_counter == self.start_stop_list[0][0]:
                self.ts_procedure = np.append(self.ts_fb, self.ts)#later we need to smooth it to deal with the break points!!!
                self.stream_fb = stumpy.stumpi(self.ts_procedure, self.m, egress=False,normalize=False) 
            #during the procedure:
            if self.start_stop_list[0][0] < self.global_counter < self.start_stop_list[0][1]:
                if self.counter > threshold:
                    self.stream_fb = copy.deepcopy(self.stream_fb_backup)
                    self.ts_procedure = copy.deepcopy(np.append(self.ts_fb, self.initial))
                    self.counter = 0
            ...
            if self.global_counter == self.start_stop_list[0][1]:
                ...
                self.stream_fb = stumpy.stumpi(ts_fb_init, self.m, egress=False,normalize=False)
    ...
    def plot(self, yellow=100, red = 200, title = "Data colored by anomaly score"):
        ...
        for left, right in self.start_stop_list_backup:
            ax.fill_between(range(left, right), v[left:right], color='orange', alpha=0.3)
